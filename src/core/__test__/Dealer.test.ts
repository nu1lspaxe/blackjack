...
describe('Dealer Class', () => {
  ...
  it('should be bust when hand value exceeds 21', () => {
    dealer.receiveCard(new Card(Suit.Clubs, Value.King));
    dealer.receiveCard(new Card(Suit.Hearts, Value.Six));
    expect(dealer.isBust()).toBe(false);
    dealer.receiveCard(new Card(Suit.Diamonds, Value.Ten));
